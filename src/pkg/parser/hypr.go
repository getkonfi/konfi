...
type HyprParser struct{}

// NewHyprParser returns a Hypr-style brace config parser.
func NewHyprParser() *HyprParser { return &HyprParser{} }

func (p *HyprParser) FindValue(data []byte, key string) (string, bool) {
	v, _, found := findResult(data, key)
	return v, found
}

func (p *HyprParser) FindLine(data []byte, key string) (int, bool) {
...
func (p *HyprParser) SetValue(data []byte, key, value string) ([]byte, error) {
	block, inner, nested := splitKey(key)
	if !nested {
...
func (p *HyprParser) DeleteKey(data []byte, key string) ([]byte, error) {
	block, inner, nested := splitKey(key)
	if !nested {
...
func (p *HyprParser) FindAll(data []byte) map[string]string {
...
func (p *HyprParser) ListKeys(data []byte) []string {
	lines := bytes.Split(data, []byte("\n"))
	var keys []string
	var stack []string
	...
	for i, line := range lines {
		...
		if inBlock {
			...
			if depth <= 0 {
				...
				lines = insertByteLine(lines, insertAt, newLine)
				return bytes.Join(lines, []byte("\n")), nil
			}
		} else {
			depth += braceBalance(trimmed)
...
func insertByteLine(lines [][]byte, at int, newLine []byte) [][]byte {
